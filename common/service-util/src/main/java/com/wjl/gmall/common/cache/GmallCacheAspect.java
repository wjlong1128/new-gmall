...
public class GmallCacheAspect {
    ...
    //  定义一个环绕通知！
    @SneakyThrows
    @Around("@annotation(com.wjl.gmall.common.cache.GmallCache)")
            ...
            if (obj == null) {
                ...
                if (result) {
                    try {
                        ...
                        if (obj == null) {
                            //  将缓存的数据变为 Json 的 字符串
                            this.redisTemplate.opsForValue().set(key, "{}", RedisConst.SKUKEY_TEMPORARY_TIMEOUT, TimeUnit.SECONDS);
                            return null;
                        }
                        //  将缓存的数据变为 Json 的 字符串
                ...
                //  直接从缓存获取的数据！
                log.info("走缓存...{}",methodSignature.getMethod().getName());
                return obj;
            }
